...
export default function DungeonsPage() {
  ...
  const [activeDungeon, setActiveDungeon] = useState<ActiveDungeonInfo | null>(null);
  ...
  const handleAbandonGate = (dungeonId: string) => {
    ...
    if (window.confirm("Are you sure you want to abandon this gate? You won't be able to re-enter.")) {
  ...
  if (loading) {
    return (
       <div className="flex flex-col items-center justify-center min-h-screen p-4">
         <p className="text-text-secondary text-lg">Checking Gate Status...</p>
       </div>
    );
  }
     ...
     return (
        <div className="flex flex-col items-center justify-center min-h-screen p-4">
            <Card className="w-full max-w-md border-danger">
              <CardHeader>
                <CardTitle className="text-danger text-center">Error Loading Gate</CardTitle>
              </CardHeader>
              <CardContent className="text-center">
                <p className="text-text-secondary mb-6">{error || 'Could not load dungeon information for this hunter.'}</p>
      ...
      <Card>
        <CardHeader className="flex flex-row justify-between items-center">
          <div>
            ...
            <CardDescription>Enter dungeons to gain experience and find loot.</CardDescription>
          </div>
        ...
        <CardContent className="space-y-6">
          {activeDungeon ? (
             ...
             <div className="text-center space-y-4">
               <h3 className="text-xl font-semibold text-primary">Active Gate Detected!</h3>
               <Card className="bg-surface/50 border-primary/60">
                 <CardHeader>
                   <CardTitle className="text-lg">{activeDungeon.type} [Rank {activeDungeon.rank}]</CardTitle>
                   <CardDescription>Current Location: Depth {activeDungeon.currentDepth} - Room {activeDungeon.currentRoom}</CardDescription>
                 </CardHeader>
                 {activeDungeon.timeRemaining && (
                   <CardContent>
                      <p className="text-sm text-accent">Time Remaining: {activeDungeon.timeRemaining}</p>
                   </CardContent>
                 )}
               </Card>
               <div className="flex flex-col sm:flex-row justify-center gap-4 pt-4">
                  <Button size="lg" glow="primary" onClick={() => handleEnterDungeon(activeDungeon.id)}>
                     Enter Dungeon
                  </Button>
                  <Button variant="destructive" size="lg" onClick={() => handleAbandonGate(activeDungeon.id)}>
                     Abandon Gate
                  </Button>
               </div>
               <p className="text-xs text-text-secondary pt-2">You must complete or abandon the current gate before opening a new one.</p>
             </div>
          ) : (
            // --- No Active Gate Display --- 
            <div className="text-center space-y-4">
              <h3 className="text-xl font-semibold text-text-secondary">No Active Gate</h3>
              <p className="text-text-secondary">Dungeons will be randomly selected based on your rank ({hunter.rank}).</p>
              {/* TODO: Maybe show list of possible dungeon types for the rank */}
              <Button size="lg" variant="secondary" glow="secondary" onClick={handleOpenNewGate}>
                 Open New Gate
              </Button>
              <p className="text-xs text-text-secondary pt-2">Opening a gate will consume [Energy/Key?] and has a time limit.</p>
            </div>
          )}
        </CardContent>
      </Card>
