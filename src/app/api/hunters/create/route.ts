...
export async function POST(request: Request) {
  ...
  if (!name || !selectedClass) {
    return NextResponse.json(
      { error: "Hunter name and class are required." },
      { status: 400 },
    );
  }
  ...
  if (!HUNTER_CLASSES[selectedClass]) {
    return NextResponse.json(
      { error: "Invalid hunter class selected." },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (existingHunters.length >= 2) {
      return NextResponse.json(
        { error: "Maximum number of hunters (2) reached." },
        { status: 400 },
      );
    }
  } catch (error) {
    ...
    return NextResponse.json(
      { error: "Could not verify hunter count." },
      { status: 500 },
    );
  }
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === "23505") {
        // Unique violation (likely name)
        return NextResponse.json(
          { error: "A hunter with this name already exists." },
          { status: 409 },
        ); // 409 Conflict
      }
      return NextResponse.json(
        { error: "Could not create hunter in database." },
        { status: 500 },
      );
    }
    ...
    return NextResponse.json(
      { message: "Hunter created successfully!", hunter: data },
      { status: 201 },
    ); // 201 Created
  } catch (error) {
