...
import { addInventoryItem, getHunterInventory } from '@/services/inventoryService';
...
export async function POST(request: Request, { params }: { params: { hunterId: string } }) {
  ...
  try {
    ...
    if (!items || items.length === 0) throw new Error('No items found in database to add randomly.');
    ...
    if (!itemIdToAdd) {
        return NextResponse.json({ error: 'Item ID to add is required.' }, { status: 400 });
    }
    ...
    if (!result.success) {
      ...
      return NextResponse.json({ error: result.error || 'Failed to add item.' }, { status: statusCode });
    }
    ...
    return NextResponse.json({ error: error.message || 'Failed to add item to inventory' }, { status: 500 });
  }
} 