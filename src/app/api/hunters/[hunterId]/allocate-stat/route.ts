...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Invalid request body. Expecting { "statName": string }.' }, { status: 400 });
  }
  ...
  try {
    ...
    if ((currentHunter.stat_points ?? 0) <= 0) {
      return NextResponse.json({ error: 'No stat points available to allocate.' }, { status: 400 });
    }
    ...
    const updatedData: { [key: string]: number } = {
      ...
      [statToAllocate]: (currentHunter[statToAllocate as keyof typeof currentHunter] ?? 0) + 1 // Increment target stat
    };
    ...
    return NextResponse.json({ error: error.message || 'Failed to allocate stat point' }, { status: 500 });
  }
} 