...
export async function POST(request: Request, { params }: { params: { hunterId: string } }) {
    ...
    try {
        ...
        if (!slot || typeof slot !== 'string' || !isEquipmentSlotType(slot)) {
            return NextResponse.json({ error: 'Valid equipment slot (string) is required to unequip an item' }, { status: 400 });
        }
        ...
        if (!unequipResult.success) {
            ...
            return NextResponse.json({ error: unequipResult.error || 'Failed to unequip item.' }, { status: statusCode });
        }
        ...
        console.error(`API Error unequipping item from slot ${params.hunterId} for hunter ${hunterId}:`, error);
        return NextResponse.json({ error: error.message || 'Failed to unequip item' }, { status: 500 });
    }
} 