...
export async function POST(
  ...
  try {
    ...
    if (!inventoryId) {
      return NextResponse.json(
        { error: "Inventory item ID (inventoryId) is required" },
        { status: 400 },
      );
    }
    if (!targetSlot) {
      return NextResponse.json(
        { error: "Target equipment slot (targetSlot) is required" },
        { status: 400 },
      );
    }
    ...
    if (!isEquipmentSlotType(targetSlot)) {
      return NextResponse.json(
        { error: `Invalid target equipment slot: ${targetSlot}` },
        { status: 400 },
      );
    }

    console.log(
      `API Request: Equip item ${inventoryId} to specific slot ${targetSlot} for hunter ${hunterId}`,
    );

    // Call the new service function (to be created)
    const result = await equipItemToSpecificSlot(
      hunterId,
      inventoryId,
      targetSlot,
    );
    ...
    if (!result.success) {
      ...
      const status = result.error?.includes("Unauthorized")
        ...
        : result.error?.includes("not found")
          ? 404
          : result.error?.includes("Cannot equip")
            ? 400 // Bad request (invalid slot for item)
            : 500; // Default to internal server error
      console.error(`API Error equipping item to slot: ${result.error}`);
      return NextResponse.json(
        { error: result.error || "Failed to equip item to slot" },
        { status },
      );
    }

    console.log(
      `API Success: Item ${inventoryId} equipped to slot ${targetSlot} for hunter ${hunterId}.`,
    );
    ...
    console.error(
      `API Error equipping item to slot for hunter ${hunterId}:`,
      error,
    );
