...
export const unlockSkill = async (
    ...
    hunterId: string,
    skillId: string
): Promise<{ success: boolean; message: string; updatedHunter?: Hunter /* Keep type but don't return */ }> => {
    ...
    try {
        ...
        if (hunterError || !dbHunter) {
            return { success: false, message: hunterError?.message || 'Hunter not found or access denied.' };
        }
        ...
        if (currentLevel < skill.levelRequirement) {
            return { success: false, message: `Level requirement not met (Req: ${skill.levelRequirement}).` };
        }
        if ((dbHunter.skill_points ?? 0) < skill.skillPointCost) {
            return { success: false, message: `Not enough skill points (Req: ${skill.skillPointCost}).` };
        }
        if (hunterRankIndex < skillRankIndex) {
            return { success: false, message: `Rank requirement not met (Req: Rank ${skill.rank}).` };
        }
        ...
        if (updateError) {
            ...
            return { success: false, message: `Failed to update hunter: ${updateError.message}` };
        }
...
export const equipSkill = async (
    ...
    try {
        ...
        if (!skill || skill.type !== 'active') {
            return { success: false, message: 'Skill not found or is not an active skill.' };
        }
        ...
        if ((dbHunter.equipped_skills?.length ?? 0) >= 4) {
            return { success: false, message: 'Maximum equipped skills reached (4).' };
        }
...
export const unequipSkill = async (
    ...
    try {
        ...
        // Update WITHOUT updated_at
        const newEquippedSkills = (dbHunter.equipped_skills || []).filter(id => id !== skillId);
        const { error: updateError } = await supabase
            .from('hunters')
            .update({
                equipped_skills: newEquippedSkills,
