...
import { calculateLevelFromExp, calculateExpForNextLevelGain, getCumulativeExpForLevelStart } from '@/lib/utils'; // Import updated leveling utils
 ...
 */
export async function processAndCombineHunterData(dbHunter: any): Promise<Hunter | null> {
  ...
  const processedHunter: Omit<Hunter, 'inventory' | 'equipment' | 'gold' | 'diamonds' | 'updatedAt' | 'unlockedSkills' | 'equippedSkills'> & { userId: string, createdAt: string } = {
    id: dbHunter.id,
  ...
  return {
    ...
    unlockedSkills: Array.isArray(dbHunter.unlocked_skills) ? dbHunter.unlocked_skills : [],
    equippedSkills: Array.isArray(dbHunter.equipped_skills) ? dbHunter.equipped_skills : [],
  } as Hunter;
}
 ...
 */
export async function deleteMyHunter(hunterId: string): Promise<{ success: boolean; error?: string }> {
  const session = await getUserSession();
  if (!session?.user) {
  ...
  console.log(`Attempting to delete hunter ID: ${hunterId} for user ID: ${userId}`);
  ...
  try {
    ...
    if (count === 0) {
        console.warn(`Hunter deletion attempt failed: Hunter ${hunterId} not found or does not belong to user ${userId}.`);
...
export async function getHunterById(hunterId: string): Promise<Hunter | null> {
    ...
    const { data: dbHunter, error } = await supabase
      ...
      if (!dbHunter) {
          console.log(`getHunterById: Hunter ${hunterId} not found or not owned by user ${userId}.`);
          return null;
      }
    ...
    console.error(`getHunterById: Unexpected error for hunter ${hunterId}:`, error);
      throw error;
    }
  }
...
export async function allocateStatPoint(hunterId: string, statName: string): Promise<{ success: boolean; updatedHunter?: Hunter; error?: string }> {
    ...
    return { success: false, updatedHunter: updatedHunter || undefined, error: 'Not implemented' };
}

export async function gainExperience(hunterId: string, amount: number): Promise<{ success: boolean; updatedHunter?: Hunter; message?: string; error?: string }> {
