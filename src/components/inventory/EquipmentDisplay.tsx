...
const EquipmentSlotDisplay: React.FC<EquipmentSlotDisplayProps> = ({ slot, item, onUnequip, onSelect, isLoading }) => {
  const rarityColor = item ? `border-${item.rarity.toLowerCase()}` : 'border-border-secondary'; // Example rarity border

  // --- DnD Droppable Setup ---
  const { setNodeRef, isOver } = useDroppable({
    // Unique ID for the droppable area
    id: `equipment-${slot}`,
    data: { // Data for the onDragEnd handler
        type: 'equipment-slot',
        slot: slot,
    },
  ...
  return (
    <div className="relative" ref={setNodeRef}> {/* Assign droppable ref */}
      <div
        className={cn(
        ...
        tabIndex={item ? 0 : -1} 
        aria-label={item ? `Equipped Item: ${item.name} in ${slot} slot. Click to view details.` : `Empty ${slot} slot. Drop equippable item here.`} // Updated label
      ...
      >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="flex flex-col items-center justify-center w-full h-full">
                  <span className="text-2xl sm:text-3xl" aria-hidden="true">{item.icon?.includes('sword') ? '⚔️' : item.icon?.includes('head') ? '👑' : item.icon?.includes('potion') ? '🧪' : item.icon?.includes('ear') ? '👂' : '📦'}</span>
                </div>
              </TooltipTrigger>
              <TooltipContent side="top" align="center">
                 <p className="font-semibold">{item.name} ({item.rarity})</p>
        ...
        {isOver && <div className="absolute inset-0 border-2 border-primary rounded-md opacity-50" />} 
      </div>
...
export const EquipmentDisplay: React.FC<EquipmentDisplayProps> = ({ equipment, onUnequip, onSelect, isLoading = {} }) => {
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-3">
                {gridLayout.flat().map((slotKey, index) => {
                    if (!slotKey) {
                        ...
                        return <div key={`placeholder-${index}`} className="w-16 h-16 sm:w-20 sm:h-20"></div>;
                    }
                    ...
                    const slotIsLoading = isLoading[slot] || (item && isLoading[item.inventoryId]);
